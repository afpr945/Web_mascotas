    ...
    <div className={className}>
      {["Proceso", "Planes", "FAQ"].map((item) => (
        <a 
          ...
          className="text-sm font-bold hover:text-sky transition-all group flex items-center gap-2"
        >
          {item}
...
const Hero = () => {
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center pt-20">
        <div className="text-left">
          ...
          <div className="flex flex-col sm:flex-row gap-6">
            ...
            <a 
              href="#proceso" 
              className="px-12 py-6 rounded-[2.5rem] border-2 border-white/20 text-white font-black text-xl hover:bg-white hover:text-navy transition-all backdrop-blur-sm text-center"
            >
              Ver Proceso →
            </a>
          </div>
        ...
        <div className="hidden lg:block relative">
          <img 
            ...
            className="w-full h-auto drop-shadow-[0_35px_35px_rgba(142,202,230,0.3)] animate-float-organico" 
            alt="Mascot" 
          />
      ...
      <div 
        className="absolute bottom-10 left-1/2 -translate-x-1/2 text-white/30 animate-bounce cursor-pointer" 
    ...
    <section className="py-40 bg-navy relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-32">
          <h2 className="font-display font-black text-5xl md:text-8xl mb-8 text-white leading-none">
            Problemas reales. <br />
            <span className="text-sky italic">Soluciones AI.</span>
          </h2>
          <p className="text-white/60 text-xl md:text-2xl font-bold max-w-3xl mx-auto leading-relaxed">
            No somos marketing tradicional. Somos ingeniería de crecimiento para mascotas.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-6">
          {items.map((item, idx) => (
            <div 
              key={idx} 
              className="group p-10 rounded-[3rem] bg-white/5 border border-white/10 hover:bg-white/10 transition-all duration-500 shadow-2xl"
            >
...
const DemoShowcase = () => (
  <section className="py-32 bg-gray-50 overflow-hidden">
    <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-20 items-center">
      <div>
        <h2 className="text-navy font-display font-black text-5xl md:text-7xl mb-8 leading-tight">
          Mira cómo <br />
          <span className="text-ocean italic">automatizamos</span> el éxito.
        </h2>
        <p className="text-gray-500 text-xl mb-12 leading-relaxed font-medium">
          Un flujo optimizado que convierte interesados en citas confirmadas mientras tú atiendes a tus clientes actuales.
        </p>
...
const Timeline = () => {
  ...
  return (
    <section id="proceso" className="py-40 bg-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-32">
          <h2 className="text-navy font-display font-black text-6xl md:text-9xl mb-12 leading-none">
            De 0 a 100 <br />
            <span className="text-ocean italic">en 7 días.</span>
          </h2>
          <p className="text-gray-400 text-2xl font-bold max-w-2xl mx-auto">
            Un proceso acelerado para que no pierdas ni un solo cliente más.
          </p>
        </div>
        ...
        <div className="relative max-w-6xl mx-auto timeline-track">
          <div className="space-y-40">
            {steps.map((step, idx) => (
              <div 
                key={idx} 
                className={`relative flex flex-col md:flex-row items-center gap-20 ${idx % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}
              >
...
const Pricing = () => {
  ...
  return (
    <section id="planes" className="py-40 bg-gray-50">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-32">
          <h2 className="text-navy font-display font-black text-5xl md:text-8xl mb-8 leading-none">
            Inversión para <br />
            <span className="text-ocean italic">Ganar.</span>
          </h2>
          <p className="text-gray-400 text-2xl font-bold">
            Sin sorpresas. Inversión inteligente que se paga con el primer cliente extra.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          {plans.map((plan, idx) => (
            <div 
              key={idx} 
              className={`flex flex-col bg-white rounded-[4rem] border-2 transition-all duration-500 hover:-translate-y-4 hover:shadow-[0_40px_80px_-15px_rgba(0,0,0,0.1)] ${plan.popular ? 'border-ocean scale-105' : 'border-gray-100'}`}
            >
              <div className={`p-16 text-center ${plan.popular ? 'bg-navy text-white' : 'bg-gray-50 text-navy'}`}>
                {plan.popular && (
                  <span className="bg-orange text-white text-[10px] font-black px-6 py-2 rounded-full mb-6 inline-block tracking-[0.2em]">
                    RECOMENDADO
                  </span>
                )}
              ...
              <div className="p-16 flex-grow flex flex-col justify-between">
                ...
                <a 
                  href={`https://wa.me/${WHATSAPP_NUM}?text=Me interesa el plan ${plan.name}`} 
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className={`block text-center py-6 rounded-3xl font-black text-xl transition-all ${plan.popular ? 'btn-primary' : 'bg-navy text-white hover:bg-ocean'}`}
                >
                  Solicitar mi demo
                </a>
              </div>
    ...
    <section id="faq" className="py-40 bg-white">
      <div className="max-w-4xl mx-auto px-6">
        <div className="text-center mb-24">
          <h2 className="text-navy font-display font-black text-6xl md:text-8xl mb-12 tracking-tighter leading-none">
            Resolvemos tus <br />
            <span className="text-sky italic">Dudas.</span>
          </h2>
        </div>
        <div className="space-y-6">
          {items.map((item, idx) => (
            <div 
              key={idx} 
              className="bg-gray-50 rounded-[3rem] overflow-hidden transition-all duration-500 hover:shadow-2xl border border-gray-100"
            >
              <button 
              ...
              >
                {item.q}
                <span className={`text-4xl transition-transform duration-500 ${openIndex === idx ? 'rotate-45 text-orange' : 'text-ocean'}`}>
                  +
                </span>
              </button>
      ...
      <div className="grid lg:grid-cols-2 gap-32 mb-32">
        <div>
          <h2 className="text-7xl md:text-9xl font-black mb-12 tracking-tighter leading-[0.85]">
            Hagámoslo <br />
            <span className="text-sky italic">Realidad.</span>
          </h2>
          <a 
        ...
        <div className="grid md:grid-cols-2 gap-20">
          ...
          <div className="space-y-10">
            ...
            <p className="text-3xl font-black leading-tight text-white/80 underline decoration-sky">
              Ventura County, <br /> California. <br /> 
              <span className="text-sky text-xl">+1 (805) 796-3581</span>
            </p>
          </div>
        </div>
      </div>
